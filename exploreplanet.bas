function ep_areaeffects(awayteam as _monster,areaeffect() as _ae,byref last_ae as short,lavapoint() as _cords,enemy() as _monster, lastenemy as short, li() as short, byref lastlocalitem as short,cloudmap() as byte) as short
...
function ep_autoexplore(awayteam as _monster,slot as short, ship as _cords,li() as short,lastlocalitem as short) as short
    ...
    lastapwp=ep_autoexploreroute(astarpath(),awayteam.c,awayteam.move,slot,ship,li(),lastlocalitem)
    if lastapwp>0 then
        for i=1 to lastapwp
...
function ep_autoexploreroute(astarpath() as _cords,start as _cords,move as short, slot as short,ship as _cords,li() as short,lastlocalitem as short) as short
    ...
    if notargets=0 then 
        ...
        if debug=1 then dprint d &":"& target.x &":"&target.y &"-"&d2 &":"& target2.x &":"&target2.y
        if d2<d then
            target.x=target2.x
            target.y=target2.y
        endif
...
function ep_checkmove(byref awayteam as _monster,byref old as _cords,key as string) as short
    ...
    if vacuum(awayteam.c.x,awayteam.c.y)=1 and vacuum(old.x,old.y)=0 and _warnings=0 then
    ...
    if tmap(awayteam.c.x,awayteam.c.y).locked>0 and rnd_range(1,6)+rnd_range(1,6)+player.science(1)+c<7+tmap(awayteam.c.x,awayteam.c.y).locked then' or (tmap(awayteam.c.x,awayteam.c.y).onopen>0 and tmap(awayteam.c.x,awayteam.c.y).locked=0) then
    ...
    if planetmap(awayteam.c.x,awayteam.c.y,slot)=54 then
        ...
        a=rnd_range(1,6)+rnd_range(1,6)+player.science(1)+c
...
function ep_display_clouds(awayteam as _monster,cloudmap() as byte,vismask() as byte) as short
    dim as short x,y,slot,osx,debug
    ...
    for x=0 to _mwx 
        for y=0 to 20
            ...
            if vismask(p.x,p.y)>0 and cloudmap(p.x,p.y)>0 then 
                if _tiles=0 then
                    put (x*_tix,y*_tiy),gtiles(gt_no(403)),trans
                else
                    set__color( 15,0)
...
function ep_display(awayteam as _monster, vismask()as byte, enemy() as _monster,byref lastenemy as short, li()as short,byref lastlocalitem as short) as short
    dim as short a,b,x,y,slot,fg,bg,osx,debug,alp
                ...
                if vismask(p2.x,p2.y)>0 then
                    for x=item(li(a)).w.x-8 to item(li(a)).w.x+8
                        for y=item(li(a)).w.y-8 to item(li(a)).w.y+8 
                            p1.x=x
                            p1.y=y
                            if distance(p1,p2)<=item(li(a)).v1/10 and tmap(x,y).seetru=0 then
    ...
    for a=0 to lastportal
        if portal(a).from.m=slot and portal(a).oneway<2 then
            ...
            if debug=1 then dprint p.x &":"&p.y
            ...
            p.y=item(li(a)).w.y
                ...
                if  (tiles(abs(planetmap(p.x,p.y,slot))).hides=0  or item(li(a)).discovered=1) and ((vismask(item(li(a)).w.x,item(li(a)).w.y)>0)) then
                    ...
                    if _tiles=0 then
                        ...
                        draw string(x*_tix,item(li(a)).w.y*_tiy),item(li(a)).w.x &":"&item(li(a)).w.y,,font1,custom,@_tcol'REMOVE
                    else
...
function ep_dropitem(awayteam as _monster, li() as short,byref lastlocalitem as short,enemy() as _monster,byref lastenemy as short,vismask() as byte) as short
    ...
    if d=1 then
        ...
        display_awayteam(awayteam,slot,0,0,ship,0,0)
        if c>0 then
...
function ep_inspect(awayteam as _monster,byref ship as _cords, enemy() as _monster, lastenemy as short, li() as short,byref lastlocalitem as short,byref localturn as short) as short
    ...
    if awayteam.c.x=ship.x and awayteam.c.y=ship.y and awayteam.c.m=ship.m then
        a=findbest(50,1)
    ...
    if (tmap(awayteam.c.x,awayteam.c.y).no>=128 and tmap(awayteam.c.x,awayteam.c.y).no<=143) or tmap(awayteam.c.x,awayteam.c.y).no=241 then 
        ...
        if tmap(awayteam.c.x,awayteam.c.y).hp=0 then
            ...
            if rnd_range(1,6)+rnd_range(1,6)+maximum(player.pilot(1)-1,player.science(1))<9 then
    ...
    for a=1 to lastenemy
        if b=0 and awayteam.c.x=enemy(a).c.x and awayteam.c.y=enemy(a).c.y then
            if enemy(a).hpmax>0 and enemy(a).hp<=0 and enemy(a).biomod>0 then
                ...
                if enemy(a).disease>0 and rnd_range(1,6)+rnd_range(1,6)+maximum(player.doctor(1),player.science(1)/2)>enemy(a).disease/2+7 then dprint "The creature seems to be a host to dangerous "&disease(enemy(a).disease).cause &"."
                if enemy(a).disease>0 and rnd_range(1,6)+rnd_range(1,6)+maximum(player.doctor(1),player.science(1)/2)+awayteam.helmet*3<enemy(a).disease then 
                ...
                if (player.science(1)>0) or (player.doctor(1)>0) then
                    ...
                    if enemy(a).slot>=0 then reward(1)=reward(1)+(10+kit+skill+addtalent(4,14,1)+enemy(a).biomod*(enemy(a).hpmax^2/3))/(planets(slot).mon_disected(enemy(a).slot)/2)
                    enemy(a).hpmax=0
                    b=1            
                    if kit>0 and rnd_range(1,6)+rnd_range(1,6)+maximum(player.doctor(1)/2,player.science(1))<9 then
                        kit=findbest(48,-1)
                ...
                if (player.science(1)>0) or (player.doctor(1)>0) then
                    ...
                    if enemy(a).biomod=0 then dprint "Dissecting a "&enemy(a).sdesc &" doesn't yield any biodata."
                else
                    dprint "No science officer or doctor in the team.",c_yel
                endif
            endif
    ...
    if b=0 and tmap(awayteam.c.x,awayteam.c.y).vege>0 and planets(slot).flags(32)<=planets(slot).life+1+addtalent(4,15,3) then
        if (player.science(1)>0) or (player.doctor(1)>0) then
            ...
            if rnd_range(1,6)+rnd_range(1,6)+skill+tmap(awayteam.c.x,awayteam.c.y).vege+addtalent(4,15,1)+kit>9+planets(a).plantsfound then
                awayteam.lastaction+=2
                planets(slot).flags(32)=planets(slot).flags(32)+1
                b=1
                dprint "you have found "&plantname(tmap(awayteam.c.x,awayteam.c.y))
                reward(1)=reward(1)+(10+kit+skill+addtalent(4,14,1)+tmap(awayteam.c.x,awayteam.c.y).vege)/planets(slot).flags(32)
                if rnd_range(1,80)-player.science(1)-addtalent(4,14,1)<tmap(awayteam.c.x,awayteam.c.y).vege then
                ...
                if rnd_range(1,100)<tmap(awayteam.c.x,awayteam.c.y).disease*2-awayteam.helmet*3 then
                    if rnd_range(1,6)+rnd_range(1,6)+maximum(player.science(1)/2,player.doctor(1))<9 then infect(rnd_range(1,awayteam.hpmax),tmap(awayteam.c.x,awayteam.c.y).disease)
                    dprint "This area is contaminated with "&disease(tmap(awayteam.c.x,awayteam.c.y).disease).ldesc
                endif
                if tmap(awayteam.c.x,awayteam.c.y).disease>0 and rnd_range(1,6)+rnd_range(1,6)+maximum(player.doctor(1),player.science(1)/2)>tmap(awayteam.c.x,awayteam.c.y).disease/2+7 then dprint "The plants here seem to be a host to dangerous "&disease(tmap(awayteam.c.x,awayteam.c.y).disease).cause &"."
    ...
    if b=0 then
        for c=1 to 9
            ...
            if tmap(p.x,p.y).turnsoninspect<>0  and rnd_range(1,6)+rnd_range(1,6)+player.science(1)>8+tmap(p.x,p.y).turnroll then
                awayteam.lastaction+=2
                b=1
    ...
    if b=0 then
        if planetmap(awayteam.c.x,awayteam.c.y,slot)=107 then
            ...
            if askyn("You could propably enhance some of the processes in this factory, to dimnish pollution. (y/n)") then
                ...
                if rnd_range(1,6)+rnd_range(1,6)+player.science(1)>9 then
...
function ep_items(awayteam as _monster, li() as short, byref lastlocalitem as short,enemy() as _monster,lastenemy as short, localturn as short,vismask()as byte,ship as _cords) as short
    ...
    for a=1 to lastlocalitem 'Drop items of dead monsters
            ...
            if debug=1 and item(li(a)).ty=18 then dprint "Rover stats:"&item(li(a)).discovered &":"& item(li(a)).w.p &":"& item(li(a)).w.s &":"& item(li(a)).v5
            if item(li(a)).ty=18 and item(li(a)).discovered=1 and item(li(a)).w.p=0 and item(li(a)).w.s>=0  and item(li(a)).v5=0 then 'Rover
                ...
                for i=1 to item(li(a)).v4
                    ...
                    if item(li(a)).vt.x=-1 then
                        if curr>last then
                            last=ep_autoexploreroute(route(),p1,item(li(a)).v2,slot,ship,li(),lastlocalitem)
                            curr=1
                        endif
                    else
...
function ep_landship(byref ship_landing as short,nextlanding as _cords,ship as _cords,nextmap as _cords,vismask() as byte,enemy() as _monster,lastenemy as short) as short
    ...
    if ship_landing<=0 then
        r=rnd_range(player.pilot(0),6)+rnd_range(1,6)+player.pilot(0)-(planets(slot).dens+planets(slot).grav*2)
        ...
        if r<0 then
            ...
            if player.hull=0 then
                ...
                if rnd_range(1,6)+rnd_range(1,6)+player.pilot(1)>10 then
...
function ep_planeteffect(awayteam as _monster, ship as _cords, enemy() as _monster, lastenemy as short,li() as short, byref lastlocalitem as short,shipfire() as _shipfire, byref sf as single,lavapoint() as _cords,vismask() as byte,localturn as short,cloudmap() as byte) as short
    ...
    if planets(slot).flags(25)=1 and awayteam.helmet=0 then
        if rnd_range(1,6)+rnd_range(1,6)+player.science(1)>15 and crew(5).hp>0 then
...
function ep_pickupitem(key as string,awayteam as _monster, byref lastlocalitem as short, li() as short,enemy() as _monster,byref lastenemy as short) as short
    dim a as short
    dim text as string
    for a=1 to lastlocalitem
        if item(li(a)).w.p=0 and item(li(a)).w.s=0 and item(li(a)).w.x=awayteam.c.x and item(li(a)).w.y=awayteam.c.y and item(li(a)).w.m=awayteam.c.m then 
            if item(li(a)).ty<>99 then 
                ...
                if item(li(a)).ty=29 then
                    if item(li(a)).v1>0 then
                        text=text &" there is something in the cage."
...
function ep_tileeffects(awayteam as _monster,areaeffect() as _ae, byref last_ae as short,lavapoint() as _cords, nightday() as byte, localtemp() as single,cloudmap() as byte,vismask() as byte) as short
            ...
            if a<tmap(x,y).causeaeon then
                ...
                if x>0 and y>0 and x<60 and y<20 then
                    if cloudmap(x,y)>0 then
                        if tmap(x-1,y).walktru=2 or tmap(x+1,y).walktru=2 or tmap(x,y-1).walktru=2 or tmap(x,y+1).walktru=2 then
                            cloudmap(x,y)-=1
                        endif
                    endif
                endif
    ...
    for a=1 to lastenemy        
        ...
        if enemy(a).hp>0 then            
            ...
            if enemy(a).aggr=1 and enemy(a).made<>101 then
                if rnd_range(1,100)>30*distance(enemy(a).c,awayteam.c) then 
                    if rnd_range(1,6)+rnd_range(1,6)>8+awayteam.invis+enemy(a).cmmod and enemy(a).sleeping<1 then
                        enemy(a).aggr=0
            ...
            if enemy(a).diet=4 and enemy(a).aggr=1 or enemy(a).aggr=2 then
                ...
                if c=-1 then
                    ...
                    next
                    enemy(a).aggr=1
                    if rnd_range(1,6)+rnd_range(1,6)+player.science(1)>3+planets(slot).atmos then dprint "Recieving radio transmission: 'Returning to ship'"
                else
                    enemy(a).target.x=item(li(c)).w.x
                    enemy(a).target.y=item(li(c)).w.y
                    if rnd_range(1,6)+rnd_range(1,6)+player.science(1)>3+planets(slot).atmos then dprint "Recieving radio transmission: 'Going for ore at "&enemy(a).target.x &":"&enemy(a).target.y &"'"
                endif
            endif
...
function ep_shipfire(shipfire() as _shipfire,vismask() as byte,enemy() as _monster,byref lastenemy as short, byref awayteam as _monster) as short
    dim as short sf2,a,b,c,x,y,r,dam,slot,osx,ani,f,debug,icechunkhole,dambonus
    dim p2 as _cords
                        ...
                        if x>=0 and y>=0 and x<=60 and y<=20 then
                            if distance(shipfire(sf2).where,p2)<=r then
                                ...
                                for a=1 to lastenemy
                                    if enemy(a).c.x=x and enemy(a).c.y=y then
                                        if shipfire(sf2).stun=0 then
                                            ...
                                            if enemy(a).stunres=0 then
                                                if dam>enemy(a).hp then enemy(a).sleeping=enemy(a).hp+dam
                                            endif
                                        endif
                                        if enemy(a).hp<=0 then
                                ...
                                if awayteam.c.x=x and awayteam.c.y=y then 
                                    ...
                                    if shipfire(sf2).stun=0 then dprint damawayteam(awayteam,dam),12
...
function ep_radio(awayteam as _monster,byref ship as _cords, byref nextlanding as _cords,byref ship_landing as short, li() as short,lastlocalitem as short,enemy() as _monster,lastenemy as short,shipfire() as _shipfire,lavapoint() as _cords, byref sf as single) as short
    ...
    if (pathblock(awayteam.c,p2,slot,1)=-1 or awayteam.stuff(8)=1 or distance(awayteam.c,p2)<2) and ship.m=slot then
        dprint "Your command?"
        text=" "
        pc=locEOL
        text=ucase(gettext(pc.x,pc.y,46,text))
        if instr(text,"ROVER")>0 then
            if debug=1 then dprint "Contacting Rover"
            b=0
            for a=0 to lastlocalitem
                if item(li(a)).ty=18 and item(li(a)).w.p=0 and item(li(a)).w.s=0 then b=li(a)
            next
            if debug=1 then dprint b &" Rover found."
            if b>0 then
                p1.x=item(b).w.x
                p1.y=item(b).w.y
        ...
        if instr(text,"GET")>0 or instr(text,"COME")>0 or instr(text,"LAND")>0 or instr(text,"RESCUE")>0 then
            if (slot=specialplanet(2) and specialflag(2)<2) or (slot=specialplanet(27) and specialflag(27)=0) or planets(slot).depth>0 then
                ...
                if instr(text,"HERE")>0 then
                    ...
                    do
                        text=cursor(nextlanding,slot,osx)
                    loop until text=key__enter or text=key__esc
            ...
            if text=key__enter then
                if pathblock(p2,shipfire(sf).where,slot,1)=0 then
                    ...
                    if shipfire(sf).when>0 then 
                        ...
                        if rnd_range(1,6)+rnd_range(1,6)+player.gunner(0)<player.weapons(shipfire(sf).what).heat/25  then
                        
                            dprint player.weapons(shipfire(sf).what).desig &" shuts down due to heat."
                            player.weapons(shipfire(sf).what).shutdown=1
                            if rnd_range(1,6)+rnd_range(1,6)+player.gunner(1)<player.weapons(shipfire(sf).what).heat/10 then
...
function ep_heatmap(awayteam as _monster, enemy() as _monster,lastenemy as short,lavapoint() as _cords,lastlavapoint as short) as short
...
function ep_grenade(awayteam as _monster, shipfire() as _shipfire, byref sf as single,li() as short ,byref lastlocalitem as short) as short
    dim as short c,slot
    slot=player.map
    dim as _cords p
    ...
    if c>0 then
        if item(c).ty=7 then
            ...
            if p.x>=0 then
                ...
                sf=sf+1
                if sf>15 then sf=0
                if item(c).v2=0 or item(c).v2=2 then
                    shipfire(sf).where=p
                        ...
                        if enemy(a).sleeping>0 then
                            if askyn("The "&enemy(a).sdesc &" is unconcious. Do you want to capture it alive?(y/n)") then
                                ...
                                if b>0 then
                                    if item(b).v1<item(b).v2 then
                                        ...
                                        enemy(a).hp=0
                                        enemy(a).hpmax=0
                                        reward(1)=reward(1)+50+cint(2*enemy(a).biomod*(enemy(a).hpmax^2)/3)
                            ...
                            if enemy(a).aggr=1 then 
                                if (askyn("Do you really want to attack the "&enemy(a).sdesc &"?(y/n)")) then
                                    ...
                                    if askyn("Do you want to switch places with the "&enemy(a).sdesc &"?(y/n)") then
...
function ep_gives(awayteam as _monster,vismask() as byte, byref nextmap as _cords, shipfire() as _shipfire,enemy() as _monster,byref lastenemy as short,li() as short, byref lastlocalitem as short,spawnmask() as _cords,lsp as short,key as string, byref ship as _cords,loctemp as single) as short
    ...
    if tmap(awayteam.c.x,awayteam.c.y).gives=27 then              
        ...
        if slot=pirateplanet(0) then
            if faction(0).war(2)<=30 then
                if rnd_range(1,100)>10+crew(2).morale+addtalent(1,4,10) then
                    dprint "Pilot "&crew(2).n &" doesnt want to come out again.",14
                    crew(2)=crew(0)
                endif
                if rnd_range(1,100)>10+crew(3).morale+addtalent(1,4,10) then
                    dprint "Gunner "&crew(3).n &" reckons he can make a fortune playing darts and decides to stay.",14
                    crew(3)=crew(0)
                endif
                if rnd_range(1,100)>10+crew(4).morale+addtalent(1,4,10) then
                ...
                if rnd_range(1,100)>10+crew(5).morale+addtalent(1,4,10) then
    ...
    if tmap(awayteam.c.x,awayteam.c.y).gives=46 then buysitems("They are very interested in buying living creatures for the zoo and arena fights."," Do you want to sell?(y/n)",26,5,0)
    ...
    if tmap(awayteam.c.x,awayteam.c.y).gives=50 then
        ...
        if planets(slot).flags(27)>0 and planets(slot).flags(26)=4 then
            ...
            player.money=player.money+50*planets(slot).flags(27)
        ...
        if planets(slot).flags(27)>0 and planets(slot).flags(26)=5 then
            ...
            player.money=player.money+250*planets(slot).flags(27)
            planets(slot).flags(27)=0
        endif
        ...
        if planets(slot).flags(27)>0 and planets(slot).flags(26)=12 then
            ...
            player.money=player.money+10*planets(slot).flags(27)
            planets(slot).flags(27)=0
        endif
        ...
        if askyn("Do you want to sell fuel for " &fuelsell &" Credits per ton? (y/n)") then
            ...
            if a>0 then
                ...
                player.money=player.money+int(a*fuelsell)
    ...
    if tmap(awayteam.c.x,awayteam.c.y).gives=56 then
        if askyn("Do you want to repair it?(y/n)") then
            a=rnd_range(1,6)+rnd_range(1,6)+player.science(1)
            if a>12 then
                dprint "You succeed!"
    ...
    if tmap(awayteam.c.x,awayteam.c.y).gives=58 then 'shutting down reactor
        if askyn("Do you want to shut it down? (y/n)") then
            for x=0 to 60
                ...
                next
            next
            if rnd_range(1,6)+rnd_range(1,6)+player.science(1)<10 then
            ...
            if tmap(awayteam.c.x,awayteam.c.y).gives=67 then
                buysitems("We buy all pieces of art you may 'find'.","Do you think you have anything to sell?",23,.25)
            endif
            ...
            if tmap(awayteam.c.x,awayteam.c.y).gives=167 then
                if askyn("A working security camera terminal. Do you want to try to use it?(y/n)") then
                    if rnd_range(1,6)+rnd_range(1,6)+player.science(1)>7 then
                        p1=awayteam.c
                        ...
                        do 
                            ...
                            ep_display (awayteam,vismask(),enemy(),lastenemy,li(),lastlocalitem)
                            display_awayteam(awayteam, slot, lastenemy, 0, ship,0,loctemp)
                            no_key=keyin
                            p2=movepoint(awayteam.c,getdirection(no_key))
                            if tmap(p2.x,p2.y).walktru=0 then awayteam.c=p2
                        loop until no_key=key__esc or rnd_range(1,6)+rnd_range(1,6)+player.science(1)<10
                        awayteam.c=p1
                    else
                        dprint "You do not get it to work properly."
                        if rnd_range(1,6)+rnd_range(1,6)+player.science(1)<11 then
                ...
                if askyn("A working security terminal. Do you want to try to use it?(y/n)") then
                    if rnd_range(1,6)+rnd_range(1,6)+player.science(1)>9 then
                        if rnd_range(1,6)+rnd_range(1,6)+player.science(1)>9 then
                            ...
                            for x=0 to 60
                                for y=0 to 20
                                    if rnd_range(1,6)+rnd_range(1,6)+player.science(1)>9 and tmap(x,y).tohit<>0 then
                                        tmap(x,y).tohit=0
                                        tmap(x,y).hitt=""
                                    endif
                                next
            ...
            if tmap(awayteam.c.x,awayteam.c.y).gives=172 then
                if askyn("A working computer terminal. Do you want to try to use it?(y/n)") then
                    if rnd_range(1,6)+rnd_range(1,6)+player.science(1)>9 then
                        ...
                        dprint "You do not get it to work properly."
                        if rnd_range(1,6)+rnd_range(1,6)+player.science(1)<13 then
                            dprint "Actually you manged to break it completely."
                            tmap(awayteam.c.x,awayteam.c.y).turnsinto=84
                        endif
                    endif
