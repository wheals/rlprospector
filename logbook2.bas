...
function logbook(byref walking as short) as short
    cls
    ...
    do
            ...
            for y=1 to ll
                if i<=last then
                    if i=curs then
                        ...
                        color 11, 0
                        if lobc(i)<>0 then color 11, lobc(i)
                    endif
                    if lobk(i)<>"" then
        ...
        if lobn(curs)>=0 then
            ...
            if key=key_walk then
                ...
                if auto_pilot(player.c,lobp(curs),diff,walking)=1 then return 0
            endif
...
function show_dotmap(x1 as short, y1 as short) as short
    ...
    for x=0 to sm_x
        for y=0 to sm_y
            if spacemap(x,y)=0 then color 0,0
            if spacemap(x,y)=1 then color 1
            if spacemap(x,y)>1 then color 9
            pset((x*2)+px,(y*2)+py)
...
function show_minimap(xx as short,yy as short) as short
    ...
    for a=0 to lastdrifting
        ...
        if planets(drifting(a).m).flags(0)=0 then
            ...
            if drifting(a).s=20 then color 15,0
