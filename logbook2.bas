...
function logbook() as short
    ...
    dim lobc(255) as short 'lobc is bg set_color(
    dim lobp(255) as _cords 'lobc is bg set_color(
...
'        'print "test for special planets and change lobc (bg set_color(): system " &map(lobn(int(a/21), (a mod 21))).desig 'debug
        if lobn(a)>=0 then
    ...
    do
            ...
            for y=1 to ll
                if i<=last then
                    if i=curs then
                        ...
                        set_color( 11, 0)
                        if lobc(i)<>0 then set_color( 11, lobc(i))
                    endif
                    if lobk(i)<>"" then
        ...
        if lobn(curs)>=0 then
            ...
            if lobn(curs)<=laststar then
                ...
                if key=key_enter and map(lobn(curs)).discovered>1 then
                for p=1 to 9
        ...
        a=0
        set_color( 11,0)
    loop until key=key_esc or player.dead<>0 or walking=10
    return 0
end function
...
function show_dotmap(x1 as short, y1 as short) as short
    ...
    for x=0 to sm_x
        for y=0 to sm_y
            if spacemap(x,y)=0 then set_color( 0,0)
            if spacemap(x,y)=1 then set_color( 1,0)
            if spacemap(x,y)>1 then set_color( 9,0)
            pset((x*2)+px,(y*2)+py)
    ...
    for a=0 to laststar
        if map(a).discovered>0 then
            set_color( spectraltype(map(a).spec),0)
            x=map(a).c.x
            y=map(a).c.y
            pset((x*2)+px,(y*2)+py)
            pset((x*2)+px,(y*2)+py+1)
...
function show_minimap(xx as short,yy as short) as short
            ...
            if x>=0 and y>=0 and x<=sm_x and y<=sm_y then
                if spacemap(x,y)>=2 and  spacemap(x,y)<=5 then 
                    if _tiles=0 then
                        ...
                        'if spacemap(x,y)=2 and rnd_range(1,6)+rnd_range(1,6)+player.pilot(0)>8 then set_color( rnd_range(48,59),1
                        'if spacemap(x,y)=3 and rnd_range(1,6)+rnd_range(1,6)+player.pilot(0)>8 then set_color( rnd_range(96,107),1
                        'if spacemap(x,y)=4 and rnd_range(1,6)+rnd_range(1,6)+player.pilot(0)>8 then set_color( rnd_range(144,155),1
                        'if spacemap(x,y)=5 and rnd_range(1,6)+rnd_range(1,6)+player.pilot(0)>8 then set_color( rnd_range(192,203),1
        ...
        if map(a).c.x>=x1-wid and map(a).c.x<=x1+wid and map(a).c.y>=y1-5 and map(a).c.y<=y1+5 then
            if map(a).discovered>0 then
                ...
                if _tiles=0 then
                    ...
                    set_color( spectraltype(map(a).spec),bg)
    ...
    for a=0 to lastdrifting
        ...
        if planets(drifting(a).m).flags(0)=0 then
            ...
            if drifting(a).s=20 then set_color( 15,0)
            if drifting(a).p>0 then
                if x>=x1-wid and x<=x1+wid and y>=y1-5 and y<=y1+5 then 
                    x=x+osx
                    y=y+osy
                    if _tiles=0 then
                        put (x*_tix+px,y*_tiy),stiles(drifting(a).g_tile.x,drifting(a).g_tile.y),pset
                    else
        ...
        if basis(a).discovered>0 then
            if basis(a).c.x>=x1-wid and basis(a).c.x<=x1+wid and basis(a).c.y>=y1-5 and basis(a).c.y<=y1+5 then
                ...
                if _tiles=1 then
                    ...
                    put (x*_tix+px,y*_tiy),gtiles(44),pset
                endif
            endif
        endif
    next
...
'    for i=1 to laststar
'        if map(i).discovered<>0 then
'            set_color( spectraltype(map(i).spec),0
'            locate map(i).c.y+1,map(i).c.x+1
'            print "*"
'        endif
'    next
