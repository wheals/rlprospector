...
function scanning() as short
    ...
    if a>0 then 
        ...
        osx=30-_mwx/2
        do
            displayplanetmap(mapslot,osx,1)
        ...
        if askyn("Do you want to refuel in the gas giants atmosphere?(y/n)") then
            ...
            if probe<=0 then
                ...
                if player.hull>0 then
                    ...
                    fuel=rnd_range(2,7)*hydrogenscoop+addtalent(2,9,3)
                    hydrogenscoop-=1
                ...
                if probeflag=1 then 
                    ...
                    dprint "The probe rerturns with "&fuel & " tons of fuel.",10
                    player.fuel=player.fuel+fuel
                    if player.fuel>player.fuelmax+player.fuelpod then player.fuel=player.fuelmax+player.fuelpod                    
                endif
            endif
...
function rg_icechunk() as short
    ...
    next b
    displayplanetmap(lastplanet,30,1)
        ...
        if d<5+dis*2 then 
            ...
            if rnd_range(1,100)<5+dis*2 then
                ...
                if askyn("Do you accept the offer?(y/n)") then
                    ...
                    if askyn("Are you certain? (y/n)") then
                        if player.money>=int(disnbase(player.c)*b) then
                            player.money=player.money-disnbase(player.c)*b
                        else
                            dprint "You convince them to lower their price. They only take all your money.",10
...
function space_radio() as short
    dim as short i,flag,closestfleet,closestdrifter
    dim as _cords p
    ...
    for i=1 to lastfleet
        if fleet(i).ty=1 or fleet(i).ty=3 or fleet(i).ty=6 or fleet(i).ty=7 then
            if distance(fleet(i).c,player.c)<df then
                df=distance(fleet(i).c,player.c)
                closestfleet=i
            endif
        endif
    next
    ...
    for i=1 to lastdrifting
        ...
        if planets(drifting(i).m).mon_template(0).made=32 and planets(drifting(i).m).flags(0)=0 then
            if distance(p,player.c)<dd then
                dd=distance(p,player.c)
                closestdrifter=i
            endif
        endif
    next
    ...
    if df<dd then
        i=closestfleet
        if i>0 then
        if distance(fleet(i).c,player.c)<1+player.sensors or debug=2 then
            flag=1
            if fleet(i).ty=1 then dodialog(5,dummy,i)
        ...
        if i>0 then
            ...
            p.x=drifting(i).x
            p.y=drifting(i).y
            if distance(p,player.c)<1+player.sensors or debug=1 then
                if planets(drifting(i).m).mon_template(0).made=32 and planets(drifting(i).m).flags(0)=0 then 
                    dodialog(4,dummy,-i)
                    flag=1
                endif
    ...
    if flag=0 then dprint "No contact"
    return 0
end function
...
function hitmonster(defender as _monster,attacker as _monster,mapmask() as byte, first as short=-1, last as short=-1) as _monster
    ...
    for a=first to noa
        if crew(a).hp>0 and crew(a).onship=0 and crew(a).disease=0 and distance(defender.c,attacker.c)<=attacker.secweapran(a)+1.5 then
            slbc+=1
            if distance(defender.c,attacker.c)>1.5 and rnd_range(1,6)+rnd_range(1,6)-player.tactic+crew(a).augment(1)+crew(a).talents(28)*3-crew(a).talents(29)*3+addtalent(3,10,1)+addtalent(3,11,1)+addtalent(a,23,1)+player.gunner(1)+attacker.secweapthi(a)+SLBonus(slbc)>9 then 
